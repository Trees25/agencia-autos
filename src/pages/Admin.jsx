...
export default function Admin() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (form.archivosImagenes && form.archivosImagenes.length > 0) {
            try { imagenesUrls = [...imagenesUrls, ...(await uploadImages())]; }
            catch { return alert('Error subiendo imágenes'); }
        }
        ...
        await supabase.from('autos').insert(autoData);

        setForm({
            marca: '', modelo: '', anio: '', km: '', motor: '', transmision: '',
            rendimiento: '', caracteristicas: '', estado: '', color: '',
            combustible: '', precio: '', status: '', imagenes: [], archivosImagenes: null
        });
        fetchAutos();
    };
            ...
            <div className="row">
                {autos.map(auto => (
                    <div key={auto.id} className="col-md-4 mb-3">
                        <div className="card">
                            {auto.imagenes?.[0] && (
                                <img src={auto.imagenes[0]} alt={`${auto.marca} ${auto.modelo}`} className="card-img-top" style={{ height: 150, objectFit: 'cover' }} />
                            )}
